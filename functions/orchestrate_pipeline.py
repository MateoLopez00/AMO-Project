...
def orchestrate_pipeline(input_midi, output_midi, comparison_xlsx="comparison.xlsx"):
    # 1. Read full MIDI and build note matrix
    _, nmat_in = read_midi_full(input_midi)
    ...
    # 4. Convert orchestrated DataFrame back to note matrix
    ...
    piano_notes = [{"pitch": row[3], "start": row[5]} for row in nmat_in]
    orch_notes  = [{"pitch": row[3], "start": row[5]} for row in nmat_out]
