...
def write_matrix_or_data(data, output_filename, midi_data=None, ticks_per_beat=480, tempo=500000):
    ...
    """
    # Full round-trip from midi_data dict
    ...
    if isinstance(data, np.ndarray):
        ...
        if cols == 9:
            if midi_data is None:
                raise ValueError("Must supply midi_data dict to round-trip a raw 9-column note matrix.")
...
from midi_io import write_matrix_or_data
...
def orchestrate_pipeline(input_midi, output_midi, comparison_xlsx="comparison.xlsx"):
    ...
    # 3. Apply orchestration (adds new_channel, new_program, new_instrument)
    df_orch = apply_orchestration(df_in)

    # 4. Convert to note-matrix array
    nmat_orch = df_orch.to_numpy()

    # 5. Write orchestrated MIDI via unified helper
    write_matrix_or_data(nmat_orch, output_midi)
    print(f"Orchestrated MIDI written to {output_midi}")

    # 6. Read back for comparison (raw nmat_out)
    _, nmat_out = read_midi_full(output_midi)

    # 7. Build and save comparison spreadsheet
    cols_out = [
    ...
    for col in cols_out[9:]: import_df[col] = ""
    export_df = df_orch.copy(); export_df.columns = cols_out

    import_df = import_df.sort_values(["in_onset_beats","in_pitch"]).reset_index(drop=True)
    export_df = export_df.sort_values(["out_onset_beats","out_pitch"]).reset_index(drop=True)
    max_len = max(len(import_df), len(export_df))
    import_df = import_df.reindex(range(max_len)); export_df = export_df.reindex(range(max_len))
